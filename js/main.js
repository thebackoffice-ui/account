...
/* Hook into switchTab so bottom nav stays in sync */
(function() {
...
(function() {
  ...
  document.addEventListener('touchend', e => {
    ...
    const sidebar = document.querySelector('.sidebar');
    if (sidebar && sidebar.classList.contains('mob-open') && dx < -50) closeMobileSidebar();
  }, { passive: true });
})();
...
function mobFixGrids() {
  ...
  statRows.forEach(row => {
    if (row.style.gridTemplateColumns && row.style.gridTemplateColumns.includes('repeat(4')) row.style.gridTemplateColumns = '1fr 1fr';
    if (row.style.gridTemplateColumns && (row.style.gridTemplateColumns.includes('1fr 220px') || row.style.gridTemplateColumns.includes('1fr 1fr 1fr') || row.style.gridTemplateColumns.includes('1fr 260px'))) row.style.gridTemplateColumns = '1fr';
  });
}
...
(function patchRenderHome() {
  ...
  if (typeof _rh === 'function') { window.renderHome = function() { _rh.apply(this, arguments); setTimeout(mobFixGrids, 50); }; }
  else { setTimeout(patchRenderHome, 500); }
})();
